...
//         // Get pointer to contiguous data
//         const uint8_t* data_ptr = reinterpret_cast<const uint8_t*>(cmd.data());

//         // Send all bytes including null terminator
//         for (size_t i = 0; i < cmd.size(); ++i) {
//             spi.Transfer(data_ptr + i, nullptr, 1);
//         }
...
// std::string read_response(hardware::SPIPeripheral& spi, const std::string& command) {
//     constexpr size_t max_response_size = 32;
//     std::vector<uint8_t> rx_buffer;
//     std::vector<uint8_t> tx_dummy(1, 0xFF); // Single dummy byte
...
//         // Print raw bytes including null
//         std::cout << "Received raw bytes: ";
//         for (auto b : rx_buffer) {
...
//             else std::cout << "\\x" << std::hex << static_cast<int>(c);
//         }
//         std::cout << "\"\n";
...
//         spi.InitPeripheral(8, 500 * 1000, 1);

//         // Read in first two bytes from Pico
//         send_command(spi, "");
...
//             // Process command with null terminator
//             std::string processed_cmd = input;
//             if (processed_cmd.empty() || processed_cmd.back() != '\0') {
//                 processed_cmd.push_back('\0');
//             }
...
//                 std::string response = read_response(spi, processed_cmd);
//             } else {
//                 send_command(spi, processed_cmd);
//             }
//         }
