#include "bothoven_hardware/pca9685/pca9685_system.hpp"
...
namespace pca9685_hardware_interface {
    hardware_interface::CallbackReturn Pca9685SystemHardware::on_init(const hardware_interface::HardwareInfo &info) {
        ...
        try {
            for (auto i = 0u; i < info_.joints.size(); i++) {
                ...
                double min_pos = (std::stod(joint.command_interfaces[0].min) * 180.0 / M_PI) + 90.0;
                double max_pos = (std::stod(joint.command_interfaces[0].max) * 180.0 / M_PI) + 90.0;
    ...
    std::vector<hardware_interface::StateInterface> Pca9685SystemHardware::export_state_interfaces() {
        ...
        for (auto i = 0u; i < info_.joints.size(); i++) {
            state_interfaces.emplace_back(hardware_interface::StateInterface(
                info_.joints[i].name, hardware_interface::HW_IF_POSITION, &hw_states_[i])); // TODO: update this to states
        }
        return state_interfaces;
    }
    ...
    hardware_interface::CallbackReturn Pca9685SystemHardware::on_configure(
        ...
        RCLCPP_INFO(rclcpp::get_logger("Pca9685SystemHardware"), "PCA Configuring...");
        try {
    ...
    hardware_interface::return_type Pca9685SystemHardware::write(const rclcpp::Time & /*time*/,
        ...
        for (auto i = 0u; i < hw_commands_.size(); i++) {
            if (current_command_values_[i] != hw_commands_[i]) {
                ...
                if (angle < min_positions_[i]) angle = min_positions_[i];
                else if (angle > max_positions_[i]) angle = max_positions_[i];
