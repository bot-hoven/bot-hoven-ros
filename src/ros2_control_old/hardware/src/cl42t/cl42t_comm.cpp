...
CL42TComm::CL42TComm()
    : spi_dev_(nullptr), bits_per_word_(0), bus_speed_hz_(0), mode_(0) {}

CL42TComm::~CL42TComm() {}

void CL42TComm::setup(hardware::SPIPeripheral *spi_dev, uint8_t bits_per_word,
                      uint32_t bus_speed_hz, int mode) {
  if (spi_dev == nullptr) {
...
void CL42TComm::send_command(std::string cmd) {
  ...
  spi_dev_->WriteData(reinterpret_cast<const uint8_t *>(cmd.c_str()),
                      cmd.size());
}

void CL42TComm::send_position(const std::string &stepper_side,
                              double position) {
...
double CL42TComm::read_position(const std::string &stepper_side) {
  ...
  try {
    ...
    spi_dev_->Transfer(reinterpret_cast<const uint8_t *>(command.c_str()),
    ...
    std::string response(reinterpret_cast<char *>(rx_buffer.data()),
                         rx_buffer.size());

    // Trim the response: remove any characters that are not digits, a decimal
    // point, or a sign.
    ...
    if (end == trimmed_response.c_str()) {
      ...
      error_message << "Failed to convert response to double: "
                    << trimmed_response;
  ...
  // // Trim the response: remove any characters that are not digits, a decimal
  // point, or a sign. std::string trimmed_response; for (char c : response) {
  //     if ((c >= '0' && c <= '9') || c == '.' || c == '-' || c == '+') {
  //         trimmed_response.push_back(c);
  //     }
  // }
  ...
  // // RCLCPP_INFO(get_logger(), "Trimmed response: %s",
  // trimmed_response.c_str());
  ...
  //     std::ostringstream error_message;
  //     error_message << "Failed to convert response to double: " <<
  //     trimmed_response; throw std::runtime_error(error_message.str());
  // }
...
//         std::ostringstream error_message;
//         error_message << "Failed to convert response to double: " <<
//         response; throw std::runtime_error(error_message.str());
